...
// 	struct tempstore tem[20];
	...
	int i=0,j=0,n1=0,n2=0,n3=0,n3s=0,n31=0,n32=0,n321=0,n33=0,p=0,p1,q=0,q1=0,qn,pl=0,plt=0,fltno=0,rasouth,phase=0,day=0,day1=0,day2=0,month=0,month1=0,month2=0,plotno=0,plotlimit=0,list=1,print,qarr[20]; //n1=dec, n2=phase,n3=month,n3s=n3south

	float xmin=0,xmax=0,ymin=0,ymax=0,t1=0,limit=10000;
	char filename[50],line[200], tem[25][50];
	float ra1[1000],ra2[1000],dec1[1000],dec2[1000],res[20][35];
...
*/	
	...
	printf("This program lists observable DEC for local time or RA between two input dates.\n Type values:\n");
...
// 	}
	...
	if((slfp = fopen("sl_table_north.txt","r")) == NULL) {
       		printf("Error: SL file not found !\n");
       		exit(1);
       	}//fopen
...
// 		printf("Month= %d: Day= %d\n",n3,n3);
		phase =0,p=0,qn=0;//,q=0;
		t1 =-1;
	   	for(n2=0;n2<15;n2++){  //15 phases
			q=0,qarr[p]=0;
			res[p][q++]=strsl[0].strph[n2].tst;
			res[p][q++]=strsl[0].strph[n2].tstp;
...
/* 				   	if(t1 == strsl[n1].strph[n2].tst) {
						printf("\t\t\t\t  Detailed: dec: %.0f to %.0f, SL =%g \n", dec1[phase],dec2[phase],count[phase] );
			        	}
					else printf(" Detailed:LT: %.1f to %.1f; ra: %.1f to %.1f, dec: %.0f to %.0f, SL =%g  ",strsl[n1].strph[n2].tst,strsl[n1].strph[n2].tstp ,ra1[phase],ra2[phase],dec1[phase],dec2[phase] ,count[phase]);
...
/*				   	if(t1 == strsl[n1].strph[n2].tst) {
					...
					else printf("Detailed:LT: %.1f to %.1f; ra: %.1f to %.1f, dec: %.0f to %.0f ,SL=%g\n",strsl[n1].strph[n2].tst,strsl[n1].strph[n2].tstp ,ra1[phase],ra2[phase],dec1[phase],dec2[phase] ,count[phase] );
...
*/
			...
			if(q>2) {
				qarr[p]=q-1;
// 				printf("q-1=%d,p=%d,qarr[p]=%d\n",q-1,p,qarr[p]);
				p++;
				p1=p;
			}


//print each 1/2 month
// 			for(q1=0;q1<q;q1++)printf("res[%d][%d] =%.1f \n",p-1,q1,res[p-1][q1]);
// 		for(p=1;p<=p1;p++) {
			if(res[p-1][2] != res[p-1][3]) {
				printf("LT: %.1f to %.1f;   RA: %.1f to %.1f : \n",res[p-1][0],res[p-1][1],res[p-1][2],res[p-1][3]);


				printf(" Dec");
			}
			j=0;
			for(q1=4;q1<=q;q1=q1+4)
				{
					if((j==0)||((res[p-1][q1+1] < 0)&& (res[p-1][q1] == 0 ))) {
						printf("  %.1f to", res[p-1][q1] );

						print =1;
					}
					if( (print ==0)&&(j!=0) &&(res[p-1][q1+1] != res[p-1][q1+4]))printf(" 2Dec  %.1f to", res[p-1][q1] );

					if(res[p-1][q1+1] != res[p-1][q1+4]) {
 						printf("  %.1f\n", res[p-1][q1+1]);
// 						printf("  [%d][%d] =%.1f\n",p-1,q1+1,res[p-1][q1+1]);
						print =0;
					}
					j++;
				}
...
for(p=0;p<p1;p++) {
	q1=0;
	printf("  %.1f   %.1f %.1f   %.1f\n", res[p][q1], res[p][q1+1], res[p][q1+2], res[p][q1+3]);
	for(q1=4;q1<qarr[p];q1=q1+4){
		printf("  %.1f   %.1f \n", res[p][q1], res[p][q1+1]);
...
*/
		if((n31 > n32) && (n3 == 24)) {  //2nd loop,if end month no > start month no
			n3 =0; 
			n321 =n32; 
		}
