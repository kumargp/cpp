...
int  main (int argc, char *argv[]) {
	...
	char *xlbl= "<== Right Ascention ==>", *ylbl="<== Declination ==>", toplbl[50];
	...
	if(((day=atoi(argv[1]))>31) || (day < 1) || ((month=atoi(argv[2]))>12) || (month < 1) || ((fltno=-1+atoi(argv[3]))>4) || (fltno < 0) || ((pl=-1+atoi(argv[4]))>2) || (pl < 0)) {

//	sscanf(date,%c%c%c%c%c,&day[0],&day[1],&month[0],&month[0],&month[1]); //third is not used
		printf("Usage:\n \t Arg 1: date (1-31),\n \t Arg2: month (1-12) \n \tArg3: filter number (1:BBF, 2:SF1, 3:SF2, 4: SF3, 5: NBF3), \n \tArg4: plot type number (1:Hammer-Aitoff, 2:Polar-North, 3:Polar-South)\n");   
...
//Projections
	...
	plotlimit=plotno;	//atoi(argv[4]);
	    for(pl=plotno;pl<=plotlimit;pl++) {
			...
			plinit();
			if(pl==0) {xylimits(&xmin,&xmax,&ymin,&ymax);}
			else {xylimits_polar(&xmin,&xmax,&ymin,&ymax);}
...
//aitoff
			if(pl==0) {
				...
				for(i=0;i<6;i++){ //objects
					fillarea(obj_ra1[i],obj_ra2[i],obj_dec1[i],obj_dec2[i],8);
					label_rd(obj_ra1[i],obj_dec1[i],object[i],objectcolr[i]);
				}
				drawgrid_ait(3);  //bracket: color code
			}
...
//polar-north
			if(pl==1) {
				pllab ("", "", toplbl);
				for(i=0;i<phase;i++){ //SL
					fillarea_polar_north(ra1[i],ra2[i],dec1[i],dec2[i],color[i]);
				}
				for(i=0;i<6;i++){  //objects
					fillarea_polar_north(obj_ra1[i],obj_ra2[i],obj_dec1[i],obj_dec2[i],8);
					label_rd_polar_north(obj_ra1[i], obj_dec1[i], object[i], objectcolr[i]);
				}
				drawgrid_north(3);  //bracket: color code
			}
//polar-south
			if(pl==2) {
				pllab ("","", toplbl);
				for(i=0;i<phase;i++){ //SL
					fillarea_polar_south(ra1[i],ra2[i],dec1[i],dec2[i],color[i]);
				}
				for(i=0;i<6;i++){
					fillarea_polar_south(obj_ra1[i],obj_ra2[i],obj_dec1[i],obj_dec2[i],8);
					label_rd_polar_south(obj_ra1[i], obj_dec1[i], object[i], objectcolr[i]);
				}
				drawgrid_south(3);  //bracket: color code
			}
	     		pladv(0);
	     		plend();
	     }
...
*/
			...
			//draw grid horizontals and verticals (color)
			// 0=black (default background),1=red(default foreground),2=yellow,3=green
			//4=aquamarine,5=pink,6=wheat,7=grey,8=brown,9=blue,10=BlueViolet,11=cyan
...
int cdfncolr = 0;
    ...
    float ALFALFA[]={};
    char *survey[] = {"HDF","CDFS","CDFN","HELLAS2XMM","Goods", "SDSS","2df", "ALFALFA"};
    int surveycolr[] ={0,0,0,0,0,0,0,0};

//eclipse
/*NOTE: need local times and to be converted to RAs
  SEPERATE FUNCTION and file
  void plot_eclips(int n3,int ecl_colr) {
    ...
    float ecl_dec2[]={};
*/
// plot_eclips(n3,ecl_colr); //n3 is month num
