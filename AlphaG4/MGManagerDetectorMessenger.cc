...
theDetector( theDet )
{
  ...
  candidates += "UWCountingFacility UWOrtecGMX SANTA Thermosiphon C14 TUNLxITEP Veto ";
  candidates += "SANTACollimator simpleString HdBEGe LNGSBEGe Giove ";
  candidates += "MGEMPI2 MGEDSG MGSOR MGEPAOLO MGEPV MGEMI MGECRIS MGEBER " ;
  candidates += "BEGe MJDemonstrator TSMJDemModInTank HADESBEGeTests AlphaDet";
  ...
  */
  ...
  fSetVisForPhysVolsMatching = new G4UIcmdWithAString("/MG/geometry/SetVisForPhysVolsMatching",this);
  fSetVisForPhysVolsMatching->SetGuidance("Set the visibility on/off for all physical volumes that ");
  fSetVisForPhysVolsMatching->SetGuidance("match the specified name. Search is case-sensitive!");
  G4UIparameter* parameter;
  G4bool omittable;
  fSetVisForPhysVolsMatching->SetParameterName("physical-volume-name-to-be-matched", omittable = false);
  ...
  fSetVisForPhysVolsMatching->SetParameter(parameter);
  parameter = new G4UIparameter("visibility", 'b', omittable = true);
  parameter->SetDefaultValue(true);
  fSetVisForPhysVolsMatching->SetParameter(parameter);
  fSetVisForPhysVolsMatching->AvailableForStates(G4State_Idle);
...
MGManagerDetectorMessenger::~MGManagerDetectorMessenger()
{
  ...
  delete fSetVisForPhysVolsMatching;
#ifndef MG_NO_G4GDML
  delete fGDMLdir;
...
void MGManagerDetectorMessenger::SetNewValue( G4UIcommand *command, G4String newValue )
{
  ...
  else if (command == fSetVisForPhysVolsMatching){
    G4String logVolName = "";
