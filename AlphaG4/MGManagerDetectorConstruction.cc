...
//
#include "lngsgesetups/MGEMPI2Detector.hh"
...
#include "geometry/AlphaDetector.hh" //added
...
MGManagerDetectorConstruction::MGManagerDetectorConstruction():
  ...
  fTheBiasedParticle("neutron"), fTheGeomSampler(NULL),
  fUseImportanceSampling(false), fUseScoring(false),
  fImpSampIsInitialized(false), fScoringIsInitialized(false),
  fTheImportanceStore(NULL),
  fMGMaterials(0), fNumberOfMaterials(0), logicWorld(NULL),
  physiWorld(NULL), theDetector(0)
{
...
MGManagerDetectorConstruction::~MGManagerDetectorConstruction()
{
  if(fTheImportanceStore) delete fTheImportanceStore;
...
//        G4cout<<(*lvciter)->GetName()<<G4endl;
        ...
        for( ipair = auxInfo.begin(); ipair != auxInfo.end(); ipair++ )
        {
          ...
          G4cout << " Valua of Auxiliary type is  =  " << val << G4endl;
...
void MGManagerDetectorConstruction::BuildAllMaterialsInDatabase()
{
  vector<string> materialNames = (MJDatabase::GetInstance())->GetMaterialNames();
  vector<string>::iterator namesIterator = materialNames.begin();
  fNumberOfMaterials = materialNames.size();
  if(MGLogger::GetSeverity() == MGLogger::debugging) {
    MGLog(trace) << "Building materials from database: " << endlog;
    while(namesIterator != materialNames.end()) {
      G4cout << *namesIterator << G4endl;
      namesIterator++;
    }
  }

  namesIterator = materialNames.begin();
  fMGMaterials = new MJMaterial*[fNumberOfMaterials];
  if(!fMGMaterials) {
    MGLog(error) << "Could not allocate memory for materials!" << endlog;
    MGLog(fatal) << endlog;
  }
  G4int i;
  for(i = 0; i < fNumberOfMaterials; i++) {
    // Horrible hack for now. G4 does not like Hg196. To be fixed...
    // RH. 7/15/2004
    if(*namesIterator != "Mercury") {
      MGLog(debugging) << "Building material : " << *namesIterator << endlog;
      fMGMaterials[i] = new MJMaterial((*namesIterator).c_str(), true);
    }
    else fMGMaterials[i] = NULL;
    namesIterator++;
  }
  MGLog(routine) << "Built all materials in database : "
		 << fNumberOfMaterials << " materials built." << endlog;
}
...
void MGManagerDetectorConstruction::SetDetector(const G4String& detectorType )
{
  if(!theDetector) {
        if (detectorType == "solidBlock")
    	  ...
    	  theDetector = new MJGeometryKURFBEGeDetectorInShield("BEGe-KURF-InShield");
	  ...
	  theDetector = new TSMJDemModInTank("TSMJDemModInTank");
    	else if (detectorType == "MGEMPI2")
    	  theDetector = new MGEMPI2Detector();
...
else if (detectorType == "AlphaDet") theDetector = new AlphaDetector("AlphaDet");
    	else
...
void MGManagerDetectorConstruction::InitializeVarianceReduction()
{
  ...
  else if(fUseImportanceSampling || fUseScoring)
  {
    ...
    else
    {
      ...
      // parallel geometry not implemented yet
    ...
    {
      fTheImportanceStore = new G4IStore(*physiWorld);
